module.exports = {
    queryTablaFormularios: function() {
        return `IF NOT EXISTS (SELECT * FROM sys.objects WHERE name='formularios' and type='U')
    ...
    querySpGeneradorProcedure: function() {
        ...
        declare @sql nvarchar(max)
                    ...
                    */
                    ...
                    +" end  IF @accion = ''editar'' begin " +trim(@SQL_UPDATE)+" end IF @accion = ''eliminar'' begin "+trim(@SQL_DELETE)+ space(1)+trim(@SQL_WHERE)
                    +" end  IF @accion = ''listar'' begin select * from "  + @P_NOMBRE_TABLA +" end;"
    ...
    querySpTablas: function() {
        return `
        SET QUOTED_IDENTIFIER OFF
        IF NOT EXISTS (SELECT * FROM sys.objects WHERE name='sp_tablas' and type='P')
            BEGIN
                declare @sql nvarchar(max)
                set @sql ='
                        create procedure [dbo].[sp_tablas](@name varchar(80)=null, @accion varchar(15)="L")
                        as 
                        begin
                        if @accion ="L"
                        begin
                        ...
                        BEGIN TRAN 
                            if (ISNULL(@name,"")="") BEGIN
                                SELECT st.[name] as table_name ,st.object_id as id            
                                FROM sys.tables st            
                                where type ="U"     
                                ORDER BY st.[name]
                            END
                                ...
                                BEGIN
                                    select  @name as [name], * from sys.all_Columns ac where ac.object_id = OBJECT_ID(@name)
